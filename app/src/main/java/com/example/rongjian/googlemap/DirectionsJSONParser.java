        ...
        try {
            ...
            for (int i = 0; i < jRoutes.length(); i++) {
                ...
                for (int j = 0; j < jLegs.length(); j++) {
                    ...
                    //Getting Distance
                    jDistance = ((JSONObject) ((JSONObject) jLegs.get(j)).getJSONObject("distance"));
                    HashMap<String, String > hmDistance = new HashMap<String, String >();
                    hmDistance.put("distance", jDistance.getString("text"));
                    //Getting Duration
                    jDuration = ((JSONObject) ((JSONObject) jLegs.get(j)).getJSONObject("duration"));
                    HashMap<String, String > hmDurantion = new HashMap<String, String >();
                    hmDurantion.put("duration", jDuration.getString("text"));

                    path.add(hmDistance);
                    path.add(hmDurantion);
